...
"""
    ...
    def grade_answer_detailed(
        ...
        prompt = f"""# 역할
당신은 한국전력공사의 수석 OPR 채점위원입니다.

# 중요한 원칙
**학생 답안이 모범답안과 거의 동일하면 90-100점을 주어야 합니다.**
**키워드 매칭은 매우 유연하게 해야 합니다.**

# 채점 기준 (총 100점)

## 1. 내용 일치도 (50점)
- 학생답안과 모범답안의 전체적인 유사도 평가
- 거의 동일: 45-50점
- 매우 유사: 40-44점
- 유사: 30-39점
- 부분 유사: 20-29점
...
## 2. 키워드 매칭 (30점)
**키워드 매칭 규칙 (매우 중요!):**
- **공백 무시**: "디지털변환" = "디지털 변환" = "디지털  변환"
- **부분 매칭**: "AI(인공지능)"이 "AI", "인공지능", "AI 기술" 등으로 포함되면 매칭
- **동의어/유사어**: "4차 산업혁명기" = "4차산업혁명" = "4차 혁명"
- **포함 관계**: "업무생산성·효율성"이 "업무생산성" 또는 "효율성" 포함 시 매칭
...
**STEP 1: 전체 유사도 판단**
- 학생답안을 읽고 모범답안과 얼마나 유사한지 판단
- 거의 같은 내용이면 "거의 동일" (45-50점)
- 70% 이상 같으면 "매우 유사" (40-44점)
...
예시:
- "A기업" → "A 기업", "A기업의" 모두 매칭
- "빅데이터" → "빅 데이터", "빅데이터 분석" 모두 매칭
- "AI(인공지능)" → "AI", "인공지능", "AI 활용" 모두 매칭
...
예시 1: 모범답안과 거의 동일한 경우 (90-100점)
```json
{{
  "총점": 95,
  "논리정확성": {{
    "점수": 48,
    "매칭된_키워드": ["키워드1", "키워드2", "키워드3", ...모든 키워드],
    "누락된_키워드": [],
    "발견된_금지어": [],
    "잘한_점": [
      "모범답안과 거의 동일한 내용과 구조",
  ...
  "종합_평가": {{
    ...
    "약점": [],
    "보완_방법": [],
    "다음_학습_방향": "현재 수준을 유지하세요"
  }}
}}
```

예시 2: 일부 키워드 누락 (70-85점)
```json
{{
  "총점": 78,
  "논리정확성": {{
    ...
    "누락된_키워드": ["키워드X", "키워드Y"],
    "발견된_금지어": [],
    "잘한_점": ["모범답안과 유사한 구조", "대부분의 키워드 포함"],
    "부족한_점": ["일부 키워드 누락"],
    "피드백": "{len(keywords)}개 중 X개 키워드 매칭"
  }},
  "명확간결성": {{
    "등급": "A",
    "점수": 25,
  ...
  "종합_평가": {{
    ...
    "약점": ["일부 키워드 누락"],
    "보완_방법": ["누락된 키워드 추가"],
    "다음_학습_방향": "키워드를 모두 포함하도록"
  }}
}}
```

**주의사항:**
- 학생답안이 모범답안과 거의 같으면 반드시 90점 이상
- 키워드 매칭은 공백/괄호를 무시하고 유연하게
- 총점 = 내용일치도(50) + 키워드매칭(30) + 형식(20) - 금지어감점
...
JSON만 출력하세요."""
        ...
        try:
            ...
            # 안전장치: 모범답안과 학생답안 유사도 체크
            # 두 답안이 70% 이상 같은데 점수가 낮으면 조정
            def text_similarity(text1: str, text2: str) -> float:
                ...
                # 포함된 문자 비율
                matched_chars = sum(1 for c in t1 if c in t2)
                return matched_chars / len(t1) if len(t1) > 0 else 0.0

            similarity = text_similarity(student_answer[:1000], model_answer[:1000])
            total_score = result.get("총점", 0)

            # 유사도가 70% 이상인데 점수가 85점 미만이면 조정
            if similarity >= 0.7 and total_score < 85:
                print(f"[WARN] 답안 유사도 {similarity*100:.1f}%인데 점수가 {total_score}점으로 낮음. 자동 조정 필요.")
                # 점수를 90-95점으로 조정
                result["총점"] = 92
                if "논리정확성" in result:
                    result["논리정확성"]["점수"] = min(result["논리정확성"].get("점수", 0) + 15, 48)
                if "명확간결성" in result:
                    result["명확간결성"]["점수"] = min(result["명확간결성"].get("점수", 0) + 5, 28)
                print(f"[INFO] 점수를 {total_score}점 → {result['총점']}점으로 조정")
